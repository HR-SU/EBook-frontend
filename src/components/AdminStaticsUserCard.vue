    ...
    <el-row>
      <el-col :span="16" :offset="4">
        <el-table :data="staticsResult" stripe
          ...
          <el-table-column prop="bookName" label="用户名" sortable></el-table-column>
          <el-table-column prop="amount" label="用户购买图书数" sortable></el-table-column>
          <el-table-column prop="spend" label="消费金额"
...
<script>
    ...
    methods: {
      ...
      handleStart: function () {
        ...
        if(this.dateRange === '' || this.dateRange === null) {
          url = 'http://localhost:9090/statics/admin/user?start='
            + '2000-01-01' + '&end='
            + '2030-01-01';
        }
        else {
    ...
    mounted() {
      let url = 'http://localhost:9090/statics/admin/user?start='
        + '2000-01-01' + '&end='
        + '2030-01-01';
      console.log(url);
      this.$axios.get(url)
